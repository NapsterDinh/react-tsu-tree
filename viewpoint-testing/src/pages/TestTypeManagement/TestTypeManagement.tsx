...
import ModalTestType from "./components/ModalTestType/ModalTestType";
import TableTestType from "./components/TableTestType/TableTestType";
import { Wrapper } from "./TestTypeManagement.Styled";

export type TestTypeTableItem = any;

const TestTypeManagement: React.FC = () => {
  ...
  const handleCallAPI = async (paginationProps?) => {
    try {
      ...
      const response = await TestTypeAPI.searchFilterTestType(dataBody);
  ...
  return (
    <Wrapper className="main-layout-wrapper">
      ...
      <BreadCrumb
        ...
        breadCrumb={true}
        link={"/test-type"}
        title={t("common:test_type_management")}
      />
      <CommonAction
        onSearch={onSearch}
