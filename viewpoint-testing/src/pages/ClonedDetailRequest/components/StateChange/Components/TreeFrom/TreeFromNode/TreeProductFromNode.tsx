...
import { OptionMenu, Wrapper } from "./TreeFromNode.Style";
...
export type TreeNodeProps = {
  ...
  setSelectedNode: (_node: TreeData) => void;
  setOpenModalDetail: (_open: boolean) => void;
};

const TreeProductFromNode = ({
  objectDataTree,
  node,
  onDelete,
  onSave,
  ...
  useEffect(() => {
    if (checkFakeNode(node) && nodeEditing?.key !== node?.key) {
      onDelete(node, true);
    }
  }, [nodeEditing?.key]);
        ...
        node?.isLocked
          ? [
              {
                key: "unlock_function",
                label: (
                  <Typography.Text>{t("common:common.unlock")}</Typography.Text>
                ),
                onClick: () => {
                  Modal.confirm({
                    width: 600,
                    ...
                    content: t("common:detail_product.modal_unlock_one"),
          ...
          : [
              ...
              },
              {
                ...
                label: (
                  <Typography.Text>
                    {t("common:common.create_new_child")}
                  </Typography.Text>
                ),
                onClick: () => {
                  onAdd(node);
              ...
              {
                key: "edit_detail_info",
                label: (
                  <Typography.Text>
                    {t("common:common.edit_detail_info")}
                  </Typography.Text>
                ),
                onClick: () => {
                  setNodeEditing(null);
              ...
              },
              {
                key: "lock_viewpoint",
                label: (
                  <Typography.Text>{t("common:common.lock")}</Typography.Text>
                ),
                onClick: () => {
...
export default React.memo(TreeProductFromNode);
