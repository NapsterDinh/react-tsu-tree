...
import { DEFAULT_FUNCTION_NODE_LEAF, GUID_EMPTY } from "@utils/constants";
import {
  deepCopy,
  loop,
  loopAllChildren,
...
import TreeProductFromNode from "./TreeFromNode/TreeProductFromNode";
...
const TreeProductFrom: React.FC<TreeComponentProps> = React.forwardRef(
  (props, ref) => {
    const {
      dataObjectTree,
      setDataObjectTree,
    ...
    const handleDropNode = React.useCallback(
      async (info) => {
        ...
        if (typeDragNode === typeDropNode) {
          ...
          showErrorNotification(t("common:detail_product.can_not_drag"));
        }
      },
    ...
    const onChangeLockedByArrayId = React.useCallback(
      async (isLocked = true) => {
        ...
        setCheckAll(false);
        if (isLocked) {
          showSuccessNotification(t("common:detail_product.lock_successfully"));
        } else {
          showSuccessNotification(
            t("common:detail_product.unlock_successfully")
          );
        }
      },
    ...
    const checkHasLockedFunctionInObjectByListId = React.useCallback(
      (listKeyNeedToCheck?: React.Key[]) => {
        ...
        return isHasLockedFunction;
      },
      [dataObjectTree?.children]
    );

    const checkHasLockedFunctionInObject = React.useCallback(
      (object: TreeData) => {
    ...
    const onDeleteByArrayId = React.useCallback(async () => {
      ...
      setCheckAll(false);
      Modal.destroyAll();
      showSuccessNotification(t("common:detail_product.delete_successfully"));
    }, [checkedList, dataObjectTree?.children]);
    ...
    const onSave = async ({
      ...
      if (checkFakeNode(node)) {
        ...
        saveNode({
          ...
          content: newContent,
          newKey: null,
          newFunctionDetail: isSaveFormData ? node.viewDetail : null,
        });
      }
    ...
    const saveNode = React.useCallback(
      ({ data, key, content, newKey, newFunctionDetail = null }) => {
        loop(data, key, (item, index, arr) => {
          const newSavedNode = {
            ...item,
            title: newFunctionDetail?.name ?? content,
            viewDetail: newFunctionDetail ?? {
              ...item.viewDetail,
              name: content,
            },
            key: newKey ?? key,
    ...
    const addNode = (node, data) => {
      if (!node?.key) {
        ...
        const newNode = {
          ...DEFAULT_FUNCTION_NODE_LEAF,
          viewDetail: {
            ...DEFAULT_FUNCTION_NODE_LEAF?.viewDetail,
    ...
    const generateNewKeyAnSaveCloneFunctionId = (object) => {
      loopAllChildren(dataObjectTree.children, (t) => {
        if (t.cloneFunctionId === object.cloneFunctionId) {
          object.cloneFunctionId = GUID_EMPTY;
          return;
        }
      });
      ...
      loopAllChildrenParamsObject(object, (item, index, obj) => {
        ...
        const newNode = {
          ...item,
          cloneFunctionId: isExist ? GUID_EMPTY : item.key,
    ...
    const transferNode = (nodeNeedTransfer) => {
      ...
      if (!nodeNeedTransfer?.parentKey) {
        ...
        newNode = {
          ...
          parentKey: null,
          cloneFunctionId: nodeNeedTransfer.key,
        };
        generateNewKeyAnSaveCloneFunctionId(newNode);

        data.push(newNode);
      } else {
        let hasParentItem = false;
        loopAllChildren(data, (item) => {
          if (item.cloneFunctionId === nodeNeedTransfer?.parentKey) {
            hasParentItem = true;
            const randomKey = randomGUID12characters();
            newNode = {
              ...newNode,
    ...
    const overrideNode = (nodeOverride) => {
      ...
      loopAllChildren(data, (item, index, arr) => {
        if (item.cloneFunctionId === nodeOverride.key) {
          const newNode = {
            ...item,
    ...
    React.useImperativeHandle(ref, () => ({
      ...
      lockUnlockAllFunction(node, isLocked) {
              ...
              {checkedList.length > 0 && (
                <>
                    ...
                    onClick={() => {
                      ...
                      Modal.confirm({
                        ...
                        title: (
                          ...
                          >
                            {t("common:detail_product.modal_lock_many")}
                            <CloseOutlined onClick={() => Modal.destroyAll()} />
                          </div>
                        ),
                        content: t("common:detail_product.modal_lock_many"),
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      Modal.confirm({
                        ...
                        content: t("common:detail_product.modal_unlock_many"),
                        cancelText: t("common:common.cancel"),
                        okText: t("common:common.ok"),
                        onOk: () => {
                  ...
                  <Button
                    ...
                    onClick={() => {
                      handleChangeNodeEditing(null);
                      if (checkHasLockedFunctionInObjectByListId(checkedList)) {
                        showInfoNotification(
                          t("common:detail_product.has_locked_function")
                        );
                        return;
                      }
                      Modal.confirm({
                        ...
                        content: t(
                          "common:detail_product.content_modal_delete"
                        ),
            ...
            <CustomRCTree
              ...
              titleRender={(node: any) => (
                <TreeProductFromNode
                  ...
                  setLockedFunction={onChangeLocked}
...
export default React.memo(TreeProductFrom);
