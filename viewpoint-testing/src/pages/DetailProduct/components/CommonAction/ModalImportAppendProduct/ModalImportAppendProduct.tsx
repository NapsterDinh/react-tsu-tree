...
import { Wrapper } from "./ModalImportAppendProductStyled";
...
const ModalImportAppendProduct: React.FC<ModalImportProductProps> = ({
  open,
  currentProduct,
  setOpen,
  getData,
  ...
  const handleOk = async () => {
    try {
      ...
      await productAPI.importAppendChild({
        payload: {
          data: data,
      ...
      await getData();
      showSuccessNotification(t("common:product.import_append_successfully"));
      setOpen(false);
      setFile(null);
      setIsEmpty(true);
      form.resetFields();
  ...
  return (
    <Modal
      forceRender
      title={t("common:product.import_append_child_product")}
      visible={open}
      width={600}
...
export default ModalImportAppendProduct;
