...
const CommonAction = ({
  ...
  setSearchParams,
  onShowCreateModal,
  onShowImportViewpointCollection,
  onDownloadViewpointTemplate,
  onResetFilter,
}) => {
  ...
  return (
    <>
      <BreadCrumb
        title={t("common:viewpoint_collection.name")}
        link="/"
      ...
      >
        ...
        <Space>
          {checkPermission(["VIEWPOINT.CREATE"]) && (
            <Button type="primary" onClick={onDownloadViewpointTemplate}>
              {t("common:common.download_template")}
            </Button>
          )}
          {checkPermission(["VIEWPOINT.CREATE"]) && (
            <Button type="primary" onClick={onShowImportViewpointCollection}>
